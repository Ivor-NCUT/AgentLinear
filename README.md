...
- **客户端配置对齐**：Codex 任务复用桌面端的内置 CLI、`CODEX_HOME`、模型、推理强度、Skills、插件配置、完整环境和本机权限，并启用相同的 code-mode host。
...
`doctor` 全程只读取本机环境，不上传代码、文件路径、登录信息或诊断结果。自检通过后，`npm start` 会打开 AgentLinear 桌面窗口；当前版本不需要 AgentLinear 云端账号。在 macOS 上，AgentLinear 优先使用 ChatGPT.app 内置的 Codex，只有未安装客户端时才回退到 PATH 中的 CLI；显式设置 `AGENTLINEAR_CODEX_PATH` 仍拥有最高优先级。
