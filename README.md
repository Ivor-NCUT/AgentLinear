...
- **上下文续聊**：Electron 保存真实 Codex Session ID，通过官方 `exec resume` 恢复同一卡片的完整上下文。
- **本地附件**：第一轮和续聊都可选择多个真实文件；Codex 获得最小目录访问权，附件路径与消息一起留存在 SQLite。
...
- Node.js 22.12 或更高版本（仓库提供 `.nvmrc`）。
...
│   ├── codex-adapter.js        Codex JSONL 执行与 Session resume
...
│   ├── codex-adapter.test.mjs  Codex JSONL 与 resume 协议测试
